...
if (!function_exists('jude_array_get_first')) {
	function jude_array_get_first(&$arr, $key=true) {
		foreach ($arr as $k=>$v) {
			break;
		}
		return $key ? $k : $v;
...
if (!function_exists('jude_array_get_keys_by_value')) {
	function jude_array_get_keys_by_value($arr, $value=1, $as_string=true) {
...
if (!function_exists('jude_array_from_list')) {
	function jude_array_from_list($arr) {
		$new = array();
...
if (!function_exists('jude_array_merge')) {
	function jude_array_merge($a1, $a2) {
...
if (!function_exists('jude_array_insert')) {
	function jude_array_insert_after(&$haystack, $needle, $stuff){
		...
		for ($i = 2; $i < func_num_args(); ++$i){
			...
			if (is_array($arg)) {
				if ($i==2)
					...
					$new_array = jude_array_merge($new_array, $arg);
			} else 
				$new_array[] = $arg;
		}
		...
		$haystack = jude_array_merge(array_slice($haystack, 0, $i, true), $new_array, array_slice($haystack, $i, null, true));
...
// @param mixed $stuff one or more arrays or scalars to be inserted into $haystack
// @return int the index at which $needle was found
if (!function_exists('jude_array_before')) {
	function jude_array_insert_before(&$haystack, $needle, $stuff){
		if (! is_array($haystack) ) return -1;
...
if (!function_exists('jude_get_tag')) {
	function jude_get_tag($text, $tag_start, $tag_end='') {
		$val = '';
...
if (!function_exists('jude_get_tag_attrib')) {
	function jude_get_tag_attrib($text, $tag, $attr) {
		$val = '';
...
if (!function_exists('jude_get_css_position_from_values')) {
	function jude_get_css_position_from_values($top='',$right='',$bottom='',$left='',$width='',$height='') {
		if (!is_array($top)) {
		...
		foreach ($top as $k=>$v) {
			$imp = substr($v, 0, 1);
			if ($imp == '!') $v = substr($v, 1);
			if ($v != '') $output .= ($k=='width' ? 'width' : ($k=='height' ? 'height' : 'margin-'.esc_attr($k))) . ':' . esc_attr(jude_prepare_css_value($v)) . ($imp=='!' ? ' !important' : '') . ';';
		}
		return $output;
	}
}
...
if (!function_exists('jude_prepare_css_value')) {
	function jude_prepare_css_value($val) {
		if ($val != '') {
			$ed = substr($val, -1);
			if ('0'<=$ed && $ed<='9') $val .= 'px';
		}
...
if (!function_exists('jude_parse_icons_classes')) {
	function jude_parse_icons_classes($css) {
		$rez = array();
...
if (!function_exists('jude_stripslashes')) {
	function jude_stripslashes($val) {
		static $magic = 0;
		if ($magic === 0) {
...
// Get GET, POST value
if (!function_exists('jude_get_value_gp')) {
	function jude_get_value_gp($name, $defa='') {
...
if (!function_exists('jude_get_value_gpc')) {
	function jude_get_value_gpc($name, $defa='', $page='', $exp=0) {
		if (isset($_GET[$name]))		 $rez = wp_unslash($_GET[$name]);
		else if (isset($_POST[$name]))	 $rez = wp_unslash($_POST[$name]);
...
if (!function_exists('jude_get_value_gps')) {
	function jude_get_value_gps($name, $defa='', $page='') {
		global $wp_session;
		if (isset($_GET[$name]))		  	$rez = wp_unslash($_GET[$name]);
		else if (isset($_POST[$name]))	  	$rez = wp_unslash($_POST[$name]);
...
if (!function_exists('jude_set_session_value')) {
	function jude_set_session_value($name, $value, $page='') {
		global $wp_session;
...
// Save value into session
if (!function_exists('jude_del_session_value')) {
	function jude_del_session_value($name, $page='') {
		global $wp_session;
...
if (!function_exists('jude_hex2rgb')) {
	function jude_hex2rgb($hex) {
		$dec = hexdec(substr($hex, 0, 1)== '#' ? substr($hex, 1) : $hex);
		return array('r'=> $dec >> 16, 'g'=> ($dec & 0x00FF00) >> 8, 'b'=> $dec & 0x0000FF);
	}
}

if (!function_exists('jude_hex2rgba')) {
	function jude_hex2rgba($hex, $alpha) {
		$rgb = jude_hex2rgb($hex);
		return 'rgba('.intval($rgb['r']).','.intval($rgb['g']).','.intval($rgb['b']).','.floatval($alpha).')';
	}
}

if (!function_exists('jude_hex2hsb')) {
	function jude_hex2hsb($hex, $h=0, $s=0, $b=0) {
		$hsb = jude_rgb2hsb(jude_hex2rgb($hex));
...
if (!function_exists('jude_rgb2hsb')) {
	function jude_rgb2hsb($rgb) {
		$hsb = array();
...
if (!function_exists('jude_hsb2rgb')) {
	function jude_hsb2rgb($hsb) {
		$rgb = array();
		$h = round($hsb['h']);
...
if (!function_exists('jude_rgb2hex')) {
	function jude_rgb2hex($rgb) {
		$hex = array(
			dechex($rgb['r']),
			dechex($rgb['g']),
			dechex($rgb['b'])
...
if (!function_exists('jude_hsb2hex')) {
	function jude_hsb2hex($hsb) {
		return jude_rgb2hex(jude_hsb2rgb($hsb));
	}
}
...
if (!function_exists('jude_prepare_macros')) {
	function jude_prepare_macros($str) {
		return str_replace(
			array("{{",  "}}",   "((",  "))",   "||"),
			array("<i>", "</i>", "<b>", "</b>", "<br>"),
			$str);
...
if (!function_exists('jude_remove_macros')) {
	function jude_remove_macros($str) {
		return str_replace(
			array("{{", "}}", "((", "))", "||"),
			array("",   "",   "",   "",   " "),
			$str);
...
if (!function_exists('jude_is_on')) {
	function jude_is_on($prm) {
		return $prm>0 || in_array(strtolower($prm), array('true', 'on', 'yes', 'show'));
	}
}
if (!function_exists('jude_is_off')) {
	function jude_is_off($prm) {
		return empty($prm) || $prm===0 || in_array(strtolower($prm), array('false', 'off', 'no', 'none', 'hide'));
	}
}
if (!function_exists('jude_is_inherit')) {
	function jude_is_inherit($prm) {
		return in_array(strtolower($prm), array('inherit'));
	}
}

// Return truncated string
if (!function_exists('jude_strshort')) {
	function jude_strshort($str, $maxlength, $add='...') {
		if ($maxlength < 0)
			return '';
		if ($maxlength == 0)
			return '';
...
if (!function_exists('jude_unserialize')) {
	function jude_unserialize($str) {
...
if (!function_exists('jude_getimagesize')) {
	function jude_getimagesize($url) {
		// Remove scheme from url
		$url = jude_remove_protocol_from_url($url);
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => jude_remove_protocol_from_url($upload_info['baseurl'])
				),
			'child' => array(
				'dir' => get_stylesheet_directory(),
				'url' => jude_remove_protocol_from_url(get_stylesheet_directory_uri())
				),
			'theme' => array(
				'dir' => get_template_directory(),
				'url' => jude_remove_protocol_from_url(get_template_directory_uri())
				)
			);
...
if (!function_exists('jude_clear_thumb_size')) {
	function jude_clear_thumb_size($url) {
		$pi = pathinfo($url);
		$pi['dirname'] = jude_remove_protocol_from_url($pi['dirname'], false);
...
if (!function_exists('jude_add_thumb_size')) {
	function jude_add_thumb_size($url, $thumb_size, $check_exists=true) {
		$pi = pathinfo($url);
		$pi['dirname'] = jude_remove_protocol_from_url($pi['dirname'], false);
		$parts = explode('-', $pi['filename']);
		...
		if ($check_exists) {
			...
			$uploads_url = jude_remove_protocol_from_url($uploads_info['baseurl'], false);
...
if (!function_exists('jude_get_thumb_size')) {
	function jude_get_thumb_size($ts) {
		static $retina = '-';
		if ($retina=='-') $retina = jude_get_retina_multiplier() > 1 ? '-@retina' : '';
		return ($ts=='post-thumbnail' ? '' : 'jude-thumb-') . $ts . $retina;
	}
}


// Return image url by attachment ID
if (!function_exists('jude_get_attachment_url')) {
	function jude_get_attachment_url($image_id, $size='full') {
		if ($image_id > 0) {
			$attach = wp_get_attachment_image_src($image_id, $size);
			$image_id = isset($attach[0]) && $attach[0]!='' ? $attach[0] : '';
		} else
			$image_id = jude_add_thumb_size($image_id, $size);
		return $image_id;
	}
}
...
if (!function_exists('jude_get_post_image')) {
	function jude_get_post_image($post_text='', $src=true) {
		global $post;
		$img = '';
...
if (!function_exists('jude_get_post_audio')) {
	function jude_get_post_audio($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = $post->post_content;
		if ($src) {
			...
			$img = jude_get_tag($post_text, '<audio', '</audio>');
			if (empty($img))
				$img = do_shortcode(jude_get_tag($post_text, '[audio', '[/audio]'));
			if (empty($img)) {
				$img = jude_get_tag_attrib($post_text, '[trx_widget_audio]', 'url');
				if (!empty($img))
...
if (!function_exists('jude_get_post_video')) {
	function jude_get_post_video($post_text='', $src=true) {
		global $post;
		$img = '';
		...
		if ($src) {
			...
			$img = jude_get_tag($post_text, '<video', '</video>');
			if (empty($img)) {
				$sc = jude_get_tag($post_text, '[video', '[/video]');
				if (empty($sc)) $sc = jude_get_tag($post_text, '[trx_widget_video', '');
...
if (!function_exists('jude_get_post_iframe')) {
	function jude_get_post_iframe($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = do_shortcode($post->post_content);
		if ($src) {
			...
			$img = jude_get_tag($post_text, '<iframe', '</iframe>');
		return $img;
	}
}

// Add 'autoplay' feature in the video
if (!function_exists('jude_make_video_autoplay')) {
	function jude_make_video_autoplay($video) {
...
if (!function_exists('jude_is_from_uploads')) {
	function jude_is_from_uploads($url) {
		$local = false;
		$url = jude_remove_protocol_from_url($url);
		$uploads_info = wp_upload_dir();
		$uploads_url = jude_remove_protocol_from_url($uploads_info['baseurl']);
		$uploads_dir = $uploads_info['basedir'];
...
if (!function_exists('jude_is_youtube_url')) {
	function jude_is_youtube_url($url) {
		return strpos($url, 'youtu.be')!==false || strpos($url, 'youtube.com')!==false;
	}
}

// Check if URL from Vimeo
if (!function_exists('jude_is_vimeo_url')) {
	function jude_is_vimeo_url($url) {
...
------------------------------------------------------------------- */
if (!function_exists('jude_init_filesystem')) {
	add_action( 'after_setup_theme', 'jude_init_filesystem', 0);
	function jude_init_filesystem() {
...
if (!function_exists('jude_fpc')) {	
	function jude_fpc($file, $data, $flag=0) {
		...
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				...
				if (jude_is_on(jude_get_theme_option('debug_mode'))) {
					...
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Put contents to the file "%s" failed', 'jude'), $file));
				}
			}
		}
		return false;
...
if (!function_exists('jude_fgc')) {	
	function jude_fgc($file) {
		global $wp_filesystem;
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				$file = str_replace(ABSPATH, $wp_filesystem->abspath(), $file);
				...
				if (jude_is_on(jude_get_theme_option('debug_mode'))) {
					// Translators: Add the file name to the message
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get contents from the file "%s" failed', 'jude'), $file));
				}
			}
		}
		return '';
...
if (!function_exists('jude_fga')) {	
	function jude_fga($file) {
		global $wp_filesystem;
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				$file = str_replace(ABSPATH, $wp_filesystem->abspath(), $file);
				...
				if (jude_is_on(jude_get_theme_option('debug_mode'))) {
					// Translators: Add the file name to the message
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get rows from the file "%s" failed', 'jude'), $file));
				}
			}
		}
		return array();
...
if (!function_exists('jude_esc')) {	
	function jude_esc($file) {
...
if (!function_exists('jude_check_min_file')) {	
	function jude_check_min_file($file, $dir) {
		if (substr($file, -3)=='.js') {
			if (substr($file, -7)!='.min.js' && jude_is_off(jude_get_theme_option('debug_mode'))) {
			...
			if (substr($file, -8)!='.min.css' && jude_is_off(jude_get_theme_option('debug_mode'))) {
				$dir = trailingslashit($dir);
...
if (!function_exists('jude_get_file_dir')) {	
	function jude_get_file_dir($file, $return_url=false) {
		// Use new WordPress functions (if present)
		if ( function_exists('get_theme_file_path') && !jude_get_theme_setting('check_min_version') ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css
			...
			if (file_exists(($child_dir).'/'.($file)))
				$dir = ($return_url ? $child_url : $child_dir).'/'.jude_check_min_file($file, $child_dir);
			else if (file_exists(($theme_dir).'/'.($file)))
				$dir = ($return_url ? $theme_url : $theme_dir).'/'.jude_check_min_file($file, $theme_dir);
		}
		return apply_filters( $return_url ? 'jude_get_file_url' : 'jude_get_file_dir', $dir, $file );
	}
}

if (!function_exists('jude_get_file_url')) {	
	function jude_get_file_url($file) {
		return jude_get_file_dir($file, true);
	}
}

// Return file extension from full name/path
if (!function_exists('jude_get_file_ext')) {	
	function jude_get_file_ext($file) {
		$parts = pathinfo($file);
...
if (!function_exists('jude_get_file_name')) {	
	function jude_get_file_name($file, $without_ext=true) {
		$parts = pathinfo($file);
...
if (!function_exists('jude_get_folder_dir')) {	
	function jude_get_folder_dir($folder, $return_url=false) {
...
if (!function_exists('jude_get_folder_url')) {	
	function jude_get_folder_url($folder) {
		return jude_get_folder_dir($folder, true);
	}
}
...
if (!function_exists('jude_remove_protocol_from_url')) {
	function jude_remove_protocol_from_url($url, $complete=true) {
...
if (!function_exists('jude_add_to_url')) {
	function jude_add_to_url($url, $prm) {
